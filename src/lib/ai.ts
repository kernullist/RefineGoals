...
export function buildSystemPrompt(): string {
  return [
    ...
    "If the user's answer is vague, partial, or says they are unsure, do not merely ask them to decide. Recommend the most practical default based on the current goal state and clearly label it as an assumption awaiting confirmation.",
    ...
    "Do not block UX options behind a prerequisite question. Show reasonable options first, then ask one follow-up question if needed.",
    ...
    '    "completenessScore": 0',
    "  },",
    '  "nextQuestions": ["one highest-priority question only"],',
    '  "suggestedChoices": [',
    '    {',
    '      "title": "Option title",',
    ...
    '      "imageQuery": "specific web image search query for a similar UX screenshot",',
    '      "visualHint": "short visual sketch description to render if no image is available"',
    "    }",
    "  ],",
    '  "suggestedArtifacts": ["PRODUCT_BRIEF.md", "TECHNICAL_SPEC.md"]',
    "}",
    "When clarification is needed, assistantMessage should use this structure in Korean: 1. understood summary, 2. unclear/conflicting points, 3. recommended default and why, 4. one question to confirm or override it.",
    "Ask exactly one clarification question when possible. Prefer the question that unblocks architecture, output format, target users, constraints, or success criteria.",
    ...
    "When the user's answer is vague, add one recommended suggestedChoice titled with '(추천)' so the user can accept it quickly.",
    "For each UX suggestedChoice, include imageQuery and visualHint. imageQuery should look for similar dashboard or product UX screenshots. visualHint should describe the layout in concrete UI terms.",
